...
export default function RegisterPage() {
  ...
  return (
    <div className="min-h-screen flex flex-col justify-center py-12 sm:px-6 lg:px-8 bg-[#050814]">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="sm:mx-auto sm:w-full sm:max-w-md">
        ...
        <h2 className="mt-8 text-center text-3xl font-extrabold tracking-tight text-white">Create an account</h2>
        ...
        <div className="glass py-10 px-8 shadow-2xl sm:rounded-[2rem]">
          <form className="space-y-6" onSubmit={handleSubmit}>
            ...
            <div>
              ...
              <Input type="email" required placeholder="you@example.com" className="h-12 bg-slate-900/60 border-slate-800"
                value={email} onChange={(e) => setEmail(e.target.value)} />
            </div>
            <div>
              <label className="block text-sm font-semibold mb-2 text-slate-200">Password</label>
              <Input type="password" required placeholder="••••••••" className="h-12 bg-slate-900/60 border-slate-800"
